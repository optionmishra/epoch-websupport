...
<main class="app-content d-flex flex-column justify-content-start">
	...
	<form id="Selform" class="p-3" name="myformsearch" method="post" action="admin_master/default_product" novalidate>
		<div class="px-1 row justify-content-center">
			...
			<div class="col-lg-2">
				<select id="select_publication" class="p-0 m-0 col-lg-12 custom-select" name="select_publication"
						required="true">
				...
				<div class="col-lg-2">
					<select id="mainSubject" class="p-0 m-0 col-lg-12 custom-select" name="mainSubject" required="true">
						...
						<?php foreach ($selectable_subjects as $msub) : ?>
							<option
								value="<?= $msub->id ?>" <?= $msub->id == $this->session->userdata('main_subject') ? ' selected="selected"' : ''; ?>><?= $msub->name ?></option>
						<?php endforeach; ?>
					</select>
				</div>
				<div class="col-lg-2">
					<select id="select_classes" class="p-0 m-0 col-lg-12 custom-select teacherClasses"
						...
						<?php foreach ($selectable_classes as $class) : ?>
							<option
								value="<?= $class->id ?>" <?= $class->id == $this->session->userdata('classes') ? ' selected="selected"' : ''; ?>><?= $class->name ?></option>
						<?php endforeach; ?>
					</select>
				</div>
			...
			<div class="col-lg-3">
					<select id="select_msubject" class="p-0 m-0 col-lg-12 custom-select teacherMsubject"
						...
						<?php foreach ($selectable_books as $book) : ?>
							...
							<option
								value="<?= $book->id ?>" <?= $book->id == $this->session->userdata('selected_book') ? ' selected="selected"' : '' ?>><?= $book->name ?></option>
					...
					<?php if ($userdata['type'] == 'Teacher') : ?>
						<ul class="row justify-content-center userTeacher">
							...
							foreach ($selectable_categories as $cat) {
								// Special case for bookorama.customercare@gmail.com - only show Flip Book category
								if ($userdata['username'] == 'bookorama.customercare@gmail.com') {
									if ($cat->name == 'Flip Book') {
										?>
										<li class="p-2 my-1 card col-5" id="active<?= $cat->id ?>"
								...
								} // Original condition for all other users
								else if ($cat->allow == 'Both' || $cat->allow == $userdata['type']) {
									?>
									<li class="p-2 my-1 card col-5" id="active<?= $cat->id ?>"
										style="border-radius:0.5rem">
										<a tab_id="<?= $cat->id ?>" class="new-search">
						...
						<ul class="row justify-content-center">
							...
							foreach ($category as $cat) {
								...
								if ($cat->allow == 'Both' or $cat->allow == $userdata['type']) {
									...
									<li class="p-2 my-1 card col-5" id="active<?= $cat->id ?>"
										style="border-radius:0.5rem">
										<a tab_id="<?= $cat->id ?>" class="new-search"><img
												src="<?= base_url('assets/cat_icons/' . $cat->icon) ?>"
												alt=""><?= $cat->name ?></a>
									</li>
									<?php
								}
								...
								<?php foreach ($online_tpgs as $tpg) : ?>
									<tr class="p-0 mt-1 mb-2 col-lg-3">
										<td class="p-0 col-lg-12">
											<div class="col-lg-12">
												<a href="<?= base_url("tpg/public/" . $tpg->book_id . "/" . $tpg->class_id) ?>"
												   class="p-0 m-0 digital-con" target="_blank">
							...
							<?php } else { ?>
								<?php foreach ($default as $def) : ?>
									...
									<?php if ($this->session->userdata('category_name') == 'Test Paper Generator') { ?>
										<div class="p-2 m-auto col-lg-10"
											 style="background: greenyellow; height: 42px; text-align: center; top: 7px; font-size: 14px;">

											<a href="<?php echo base_url(); ?>query/teacher-question" target="_blank"
											   style="color: #444;font-weight: 600;">Submit Your Question</a>
